...
function getEndpoint() {
  return process.env.NEXT_PUBLIC_FORMSPREE_TRIAL_ENDPOINT || "";
}
...
export default function TrialPage() {
  ...
  if (submitted) {
    return (
      <Success title={t.forms.successTitle[lang]} body={t.forms.successBody[lang]} />
    );
  }
    ...
    <div className="space-y-8">
      <header className="rounded-2xl bg-white p-6 shadow-soft ring-1 ring-black/5 md:p-8">
        <h1 className="text-2xl font-extrabold">
          {lang === "ko" ? "무료 체험 수업 신청" : "Book a Free Trial"}
        </h1>
      ...
      {!endpoint && (
        <div className="rounded-2xl border border-black/10 bg-sun/40 p-5">
          ...
          <p className="mt-2 text-sm">
            {lang === "ko" ? `전화: ${branch.phone}` : `Call: ${branch.phone}`}
          </p>
        </div>
      )}
      ...
      >
        ...
        <div className="mt-4 grid gap-4">
          <Field label={lang === "ko" ? "메모" : "Notes"}>
            <textarea
              ...
              required
              className="input"
              placeholder={lang === "ko" ? "현재 수준/목표를 적어주세요." : "Current level, goals, anything we should know."}
            />
          </Field>
        </div>

        <div className="mt-5 flex items-start gap-3">
          <input id="trial-consent" name="consent" type="checkbox" required className="mt-1 h-4 w-4 accent-ink" />
          <label htmlFor="trial-consent" className="text-sm text-black/70">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Button type="submit" disabled={!endpoint}>
            {lang === "ko" ? "체험 수업 신청" : "Request Trial"}
          </Button>
